/**
 * ApiRunHistoryPage — DataTable of past API integration runs for an adapter.
 *
 * Route: /m/api_integration/adapters/:code/runs
 * Backend: GET /api/api_integration/api/v1/G/adapters/:code/runs
 */
...
const ApiRunHistoryPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .get(`/api/api_integration/api/v1/G/adapters/${encodeURIComponent(code)}/runs`)
      .then((res) => {
        if (cancelled) return;
  ...
  const columns = useMemo<Column<ApiRun>[]>(
    () => [
      {
        key: 'hashId',
        header: 'Run',
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center gap-3">
        <Link to="/m/api_integration" className="text-gray-500 hover:text-gray-800 inline-flex items-center gap-1 text-sm">
      ...
      <DataTable
        ...
        onRowClick={(r) =>
          navigate(`/m/api_integration/adapters/${encodeURIComponent(code || '')}/runs/${encodeURIComponent(r.hashId)}`)
        }
      />
    </div>
  );
...
export default ApiRunHistoryPage;
