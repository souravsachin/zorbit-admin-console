/**
 * ApiRunLauncherModal — launches a new API integration run for an adapter.
 *
 * POST /api/api_integration/api/v1/G/adapters/:code/runs body {trigger, payload}.
 */
...
const ApiRunLauncherModal: React.FC<Props> = ({ adapter, onClose, onLaunched }) => {
  ...
  const submit = async () => {
    ...
    try {
      const res = await api.post(
        `/api/api_integration/api/v1/G/adapters/${encodeURIComponent(adapter.code)}/runs`,
        { trigger, payload },
      );
...
export default ApiRunLauncherModal;
