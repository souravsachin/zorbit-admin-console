/**
 * RpaRunConsolePage — live SSE console for a single RPA run.
 *
 * Connects to /api/rpa_integration/api/v1/G/adapters/:code/runs/:runId/events
 * via EventSource and renders a kernel-style log tail.
 *
 * Events handled:
 *   - 'event'    — IntegrationRunEvent (ts, level, message, data)
 *   - 'complete' — run terminated (status, error)
 *
 * Route: /m/rpa_integration/adapters/:code/runs/:runId
 */
import React, { useEffect, useMemo, useRef, useState } from 'react';
import { useParams, Link } from 'react-router-dom';
import { Cpu, ChevronLeft, Loader2, CheckCircle2, XCircle } from 'lucide-react';
...
const RpaRunConsolePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const url = `/api/rpa_integration/api/v1/G/adapters/${encodeURIComponent(code)}/runs/${encodeURIComponent(runId)}/events`;
    ...
    const onError = () => {
      setStatus('error');
      // Let EventSource retry; we just surface the state
    };
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link
            to={`/m/rpa_integration/adapters/${encodeURIComponent(code || '')}/runs`}
...
export default RpaRunConsolePage;
